...
2. **单篇批改**：上传图片（支持 1~3 张） → 选择学生/OCR 引擎/文体 → 异步批改 → 查看结果
3. **批量批改**：扫描指定日期目录，按 `人名_作文题目_序号.jpg` 规则自动识别并批改，同一姓名、题目下最多 3 张图片将按序号拼接成一篇文章
4. **定时自动扫描**：每天指定时间自动扫描当天目录
5. **OCR 引擎**：可选择 Kimi 视觉识别、百度手写 OCR、腾讯手写 OCR
6. **文体模板**：记叙文、议论文、说明文、散文，支持不选择文体，按通用要求批改
...
```
陈宏毅_忙节奏让我找到了松弛感_1.jpg
陈宏毅_忙节奏让我找到了松弛感_2.jpg
陈宏毅_忙节奏让我找到了松弛感_3.jpg
```

系统会：
1. 根据文件名解析出人名、作文题目和序号
2. 将同一姓名、同一题目的图片按序号从小到大组合成一篇文章（最多 3 张）
3. 自动查找或创建学生
4. 复制图片到 uploads 目录
5. 异步进行 OCR 识别和作文批改
...
| POST | `/api/essays/upload` | 上传作文图片（单张或多张，最多 3 张） |
